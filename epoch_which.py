...
def train_change(epochn,topk,model_dict):
    def setup_seed(seed):
    ...
    train_X, train_y, train_z, test_X, test_y, test_z = getdata.getdata_train(WAV_PATH)
    print(train_X.shape)
    ...
    for epoch in range(epochs):
        ...
        train_loss=all_loss /(len(train_loader)-1)/BATCH_SIZE
        # 测试
        evalidate = []
        model.eval()
        acc = 0.0
