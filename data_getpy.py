...
class get_data(object):
    ...
    def getdata_train(self, path, RATE=16000):  # t=2 按照时间间隔为2秒来进行划分
        ...
        train_files=[]
        #path_math="save_top_no_model/1epoch_1train_alltest/save_top317_{}_new_reverse.json".format(self.topk)
        path_math = "save_top_no_model/epoch5/save_top1425_{}_new_reverse.json".format(self.topk)
    ...
    def getdata_Tracin(self, path, RATE=16000):  # t=2 按照时间间隔为2秒来进行划分
        ...
        assert len(train_X) == len(train_y), "X length and y length must match! X shape: {}, y length: {}".format(
            train_X.shape, train_y.shape)
        return train_X, train_y, train_z, test_X, test_y, test_z
    def getdata_retrain(self, path, RATE=16000):
        '''处理的RAVDESS数据'''
        #  把每条语句切分成多个2秒的片段，片段之间存在1秒的重叠（测试集是1.6秒重叠）
        path = path.rstrip('/')  # 删除路径后面的'/'符号
    ...
    def getdata_train_leitop(self, path, RATE=16000):  # t=2 按照时间间隔为2秒来进行划分
        ...
        train_files=[]
        path_math1= "save_top/epoch2/01/save_top587_{}_new_reverse_epoch2.json".format(self.topk)
        path_math2 = "save_top/epoch2/04/save_top323_{}_new_reverse_epoch2.json".format(self.topk)
        path_math3 = "save_top/epoch2/05/save_top168_{}_new_reverse_epoch2.json".format(self.topk)
        path_math4 = "save_top/epoch2/07/save_top344_{}_new_reverse_epoch2.json".format(self.topk)
        train_files_all = glob.glob("Train/" + '/*.wav')  # 获取声音文件
        valid_files = glob.glob("Test/" + '/*.wav')  # 获取声音文件
        #'''去所有数据中，四个类别分别取前10，这TRACIN是所有train对所有test的'''
        # lei1=[]
        # lei2 = []
        # lei3 = []
        # lei4 = []
        ...
        #         else:
        #             continue
        '''Tracin是同类train度同类的test数据的，最后测试数据还是全部放在一起'''
        with open(path_math1,'r') as text1:


            with open(path_math2,'r') as text2:
                with open(path_math3) as text3:
                    with open(path_math4) as text4:
                        text1=str(text1.readlines())
                        text2=str(text2.readlines())
                        text3 = str(text3.readlines())
                        text4 =str(text4.readlines())
                        for i in train_files_all:

                            name=str(os.path.basename(i).split('\\')[0])
                            if (name in text1) or (name in text2) or (name in text3) or (name in text4):
                                train_files.append(i)
                            else:
                                continue
        train_X = []
        train_y = []
        train_z = []
        print("constructing meta dictionary for {}...".format(self.topk))
        # 这里的enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，
        for i, wav_file in enumerate(tqdm(train_files)):  # 这里是对于训练数据来处理
            label = str(os.path.basename(wav_file).split('-')[2])  # 函数返回path最后的文件名，获取标签的名称
    ...
    def getdata_checkpoint(self, path, RATE=16000):
        '''处理的原始训练模型，共计算三个checkpoint'''
        #  把每条语句切分成多个2秒的片段，片段之间存在1秒的重叠（测试集是1.6秒重叠）
        path = path.rstrip('/')  # 删除路径后面的'/'符号
        ...
        valid_files = glob.glob('Test' + '/*.wav')

        train_X = []
        train_y = []
        train_z = []
        print("constructing meta dictionary for checkpoint...")
        # 这里的enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，
        for i, wav_file in enumerate(tqdm(train_files)):  # 这里是对于训练数据来处理
            label = str(os.path.basename(wav_file).split('-')[2])  # 函数返回path最后的文件名，获取标签的名称
        ...
        assert len(train_X) == len(train_y), "X length and y length must match! X shape: {}, y length: {}".format(
            train_X.shape, train_y.shape)
        return train_X, train_y, train_z, test_X, test_y, test_z
    def getdata_train_huadong(self, path, RATE=16000):  # t=2 按照时间间隔为2秒来进行划分
        #  把每条语句切分成多个2秒的片段，片段之间存在1秒的重叠（测试集是1.6秒重叠）
        path = path.rstrip('/')  # 删除路径后面的'/'符号
